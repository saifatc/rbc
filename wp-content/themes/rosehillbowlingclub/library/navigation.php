...
register_nav_menus( array(
    ...
    'quick-links' => esc_html__( 'Quick Links', 'foundationpress' ),
    'our-facilities' => esc_html__( 'Our Facilities', 'foundationpress' ),
    'related-links' => esc_html__( 'Related Links', 'foundationpress' ),    
));
 ...
 * @link http://codex.wordpress.org/Function_Reference/wp_nav_menu
 */
if ( ! function_exists( 'our_facilities' ) ) {
	function our_facilities() {
		wp_nav_menu( array(
			'container'      => false,
			'menu_class'     => 'footer-menu',
			'items_wrap'     => '<ul id="%1$s" class="%2$s">%3$s</ul>',
			'theme_location' => 'our-facilities',
			'depth'          => 3,
			'fallback_cb'    => false,
//			'walker'         => new Foundationpress_Top_Bar_Walker(),
		));
